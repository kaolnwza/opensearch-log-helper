...
// Where the normalised store lives in chrome.storage.local. The popup reads it
// too — it offers the same container names the in-page editor completes.
var LF_FORMS_KEY = "filterForms";
...
// The container names worth completing are the ones deployed on *this*
// cluster, so the editor's value suggestions belong in the file that is
// already per-environment, gated by the same `dns` as the forms beside them.
// Named apart from suggest-presets.js's LF_SUGGEST_KEY — both files share one
// global scope in the content script.
var LF_FORM_SUGGEST_KEY = "suggest";
...
// A form may name the fields its rows should show. Specs are handed to the
// extractor as written — `coalesce(a, b)` and friends are its grammar, parsed
// there — so only the shape is checked here.
function lfNormalizeFieldSpecs(raw) {
...
// `suggest` is field → values, and the suggestion code iterates those arrays,
// so a bad edit must come back as an empty list rather than something it can
// crash on. An entry that survives with no field at all is not worth keeping.
function lfSuggestEntry(rawValues, dns) {
    if (!rawValues || typeof rawValues !== "object" || Array.isArray(rawValues))
        return null;
    const values = {};
    for (const [field, list] of Object.entries(rawValues)) {
        const f = typeof field === "string" ? field.trim() : "";
        if (!f || !Array.isArray(list)) continue;
        const vals = [];
        for (const v of list) {
            const s = typeof v === "string" ? v.trim() : "";
            if (s && !vals.includes(s)) vals.push(s);
        }
        if (vals.length) values[f] = vals;
    }
    return Object.keys(values).length
        ? { values, dns: lfNormalizeHosts(dns) }
        : null;
}

// Storage keeps the already-flattened list, so it comes back in through here
// rather than through the file normaliser below.
function lfNormalizeSuggests(raw) {
    const out = [];
    for (const s of Array.isArray(raw) ? raw : []) {
        const entry = lfSuggestEntry(s?.values, s?.dns);
...
function lfNormalizeFilterForms(raw) {
    ...
    const push = (name, dsl, dns, color, fields) => {
        if (typeof name !== "string" || !name.trim()) return void skipped++;
        if (!dsl || typeof dsl !== "object" || Array.isArray(dsl))
            return void skipped++;
        const entry = withColor(
            { name: name.trim(), dsl, dns: lfNormalizeHosts(dns) },
            color,
        );
        const specs = lfNormalizeFieldSpecs(fields);
        if (specs.length) entry.fields = specs;
        entries.push(entry);
    };
    ...
    const pushSuggest = (rawValues, dns) => {
        const entry = lfSuggestEntry(rawValues, dns);
        if (entry) suggests.push(entry);
        else skipped++;
    };
    ...
    // A whole file may share one field list; a group or a form still overrides it
    const fileFields = Array.isArray(raw) ? undefined : raw?.fields;
    ...
    if (list) {
        for (const e of list) {
            ...
            // A group: several forms, links and suggestion values sharing one
            // dns, so a single file can carry a section per environment. An
            // entry may still override it.
            if (
                Array.isArray(e.forms) ||
                Array.isArray(e[LF_LINK_KEY]) ||
                (e[LF_FORM_SUGGEST_KEY] && typeof e[LF_FORM_SUGGEST_KEY] === "object")
            ) {
                const gFields = e.fields !== undefined ? e.fields : fileFields;
                const fColor =
                ...
                for (const f of e.forms || []) {
                    if (!f || typeof f !== "object" || Array.isArray(f)) skipped++;
                        ...
                        push(
                            ...
                            f.fields !== undefined ? f.fields : gFields,
                        );
                }
                for (const l of e[LF_LINK_KEY] || []) pushLink(l, e.dns, lColor);
                if (e[LF_FORM_SUGGEST_KEY] !== undefined)
                    pushSuggest(e[LF_FORM_SUGGEST_KEY], e.dns);
                continue;
            }
            push(e.name, e.dsl, e.dns, e.color, e.fields !== undefined ? e.fields : fileFields);
        }
    } else if (raw && typeof raw === "object") {
        // name → clause map: `dns`, `fields`, `suggest` and the colour keys at
        // this level gate, paint and shape the file, they are not forms
        for (const [k, v] of Object.entries(raw)) {
            if (
                ...
                k === LF_LINK_KEY ||
                k === LF_FORM_SUGGEST_KEY
            )
                continue;
            push(k, v, undefined, undefined, raw.fields);
        }
    } else {
        return {
            forms: [],
    ...
    // …and so may one `suggest` block for the whole file, taking the file's dns
    if (!Array.isArray(raw) && raw?.[LF_FORM_SUGGEST_KEY] !== undefined)
        pushSuggest(raw[LF_FORM_SUGGEST_KEY], raw.dns);
