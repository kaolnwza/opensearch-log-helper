...
- **Editor suggestion presets** — one autocomplete preset per project, saved in extension storage:
  ...
  The filter forms file's `suggest` block (below) and the values found in the loaded logs are
  suggested on top of the preset's own list.
- **Filter forms** — load a `.json` file of named Query DSL clauses in the panel and click one to run
  ...
  A form may also carry `fields` — the extractor columns to show while that form is on, since one
  button usually means one shape of log:

  ```json
  {
    "name": "drawdown 621814020349",
    "dsl": { "match": { "json_payload": "621814020349" } },
    "fields": ["message.code", "message.data"]
  }
  ```

  Clicking the form runs the query and repaints the overlay with exactly those fields; clicking the
  lit button again puts the panel's own list back. The list is **not** saved over your `extractFields`
  — it lasts as long as the form is on. `fields` inherits like the rest (form → group → file) and the
  entries are extractor specs, so `coalesce(a, b)` works here too.
...
- **Suggestion values from the forms file** — container names are per-cluster, so the file that is
  already gated by `dns` is where they live. A `suggest` block maps a field path to the values the
  in-page editor (and the popup's Container Name box) offers for it:
  ...
  [
    {
      "dns": ["logs.prod.example"],
      "suggest": {
        "kubernetes.container_name": ["core-account-accept", "orch-schedule"]
      },
      "forms": [{ "name": "prod errors", "dsl": { "match_phrase": { "level": "ERROR" } } }]
    }
  ]
  ...
  The block takes its group's `dns`, so a prod group and a dev group each offer their own container
  names on their own host. It also works at the top level of a `{ "forms": … }` object or a
  `{ "name": clause }` map file, where it takes the file's `dns` and `suggest` is never read as a
  form name. Several visible groups add up rather than the first one winning, and the preset's own
  values and the values seen in the loaded logs are merged in on top. A file of nothing but a
  `suggest` block is fine.
