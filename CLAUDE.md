...
- **`chrome.storage.local`** — anything the popup and the page both need: `containerTags`,
  `payloadTags`, `payloadMode`, `extractFields`, `extractAuto`, `theme`, and `suggestPresets`
  (the `LF_SUGGEST_KEY` store). `content.js` subscribes to `chrome.storage.onChanged` for
  `suggestPresets`, so preset edits apply live without a reload.
...
`suggest-presets.js` loads before `content.js` and is also `<script>`-included by `popup.html`, so both
sides share `lfNormalizeStore()` / `lfBuiltinPreset()`. Every read of the preset store must go through
`lfNormalizeStore` — it tolerates whatever an older version left behind and always yields at least one
preset. Changing the stored shape means changing it in that one file.
