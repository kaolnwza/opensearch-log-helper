...
function removeFieldFromQuery(q, field) {
  ...
  q = q.replace(
    new RegExp(`\\s*(?:AND|OR)?\\s*${esc}:${val}`, "gi"), ""
  );

  // 3. Clean up dangling operators and whitespace
  q = q.replace(/^\s*(?:AND|OR)\s+/i, "").replace(/\s+(?:AND|OR)\s*$/i, "").trim();
  return q;
}
...
function expandAll() {
  ...
  if (!toggles.length) return { ok: false, error: "No expand buttons found on this page." };
  let expanded = 0;
  toggles.forEach((btn) => {
    const isExpanded =
      btn.getAttribute("aria-expanded") === "true" ||
      btn.classList.contains("opened");
    if (!isExpanded) { btn.click(); expanded++; }
  });
...
function addFilter({ field, value, mustNot, mode }) {
  ...
  if (!input) return { ok: false, error: "Query bar not found. Make sure you're on the Discover page." };

  let clause = mode === "wildcard" ? `${field}:*${value}*` : `${field}:"${value}"`;
...
function clearFilters() {
  const input = findElement(SELECTORS.queryInput);
  if (input) { input.focus(); setNativeValue(input, ""); }
  ...
  setTimeout(() => {
    ...
    else if (input) input.dispatchEvent(new KeyboardEvent("keydown", { key: "Enter", keyCode: 13, bubbles: true }));
  }, 150);
...
function buildClause(field, term, mode) {
  switch (mode) {
    case "term":     return `${field}:"${term}"`;
    case "term_not": return `NOT ${field}:"${term}"`;
    case "regex":    return `${field}:/.*${term}.*/`;
    case "wildcard": return `${field}:*${term}*`;
    case "match":    return `${field}:${term}`;
    default:         return `${field}:/.*${term}.*/`;
  }
}
...
function addMultiFilter({ field, terms, op, mode = "regex" }) {
  ...
  const group = clauses.length > 1 ? `(${clauses.join(` ${op} `)})` : clauses[0];
...
function saveColWidths(w) {
  try { localStorage.setItem(COL_WIDTH_KEY, JSON.stringify(w)); } catch {}
}
...
function applyColWidth(fieldName, px) {
  ...
  document.querySelectorAll(`[data-lf-field="${fieldName}"]`).forEach((el) => {
...
function purgeOverlays() {
  document.querySelectorAll(`.${EXTRACT_ROW_CLASS}`).forEach((el) => el.remove());
  document.querySelectorAll("[data-lf-key]").forEach((el) => el.removeAttribute("data-lf-key"));
}
...
function tryParseJson(text) {
  ...
  for (const sfx of ['"}', '"}}', '"}}}', "}}", "}}}", "}}}}"]) {
    try { return JSON.parse(s + sfx); } catch {}
  }
  return null;
}
...
try { activeTheme = localStorage.getItem("lf_theme") || "light"; } catch {}
function T() { return THEMES[activeTheme] || THEMES.light; }
...
function syntaxHighlight(obj, rawFallback) {
  ...
  return safe.replace(
    ...
    (match) => {
      ...
      if (match === "true")  return `<span style="color:${T().bool_t}">${match}</span>`;
      if (match === "false") return `<span style="color:${T().bool_f}">${match}</span>`;
      if (match === "null")  return `<span style="color:${T().toggle}">${match}</span>`;
...
function parseSpec(spec) {
  ...
  if (m) return { type: "coalesce", fields: m[1].split(",").map((f) => f.trim()) };
...
function resolveSpec(spec, parsed, regexResult) {
  ...
  if (p.type === "coalesce") {
    for (const f of p.fields) {
      ...
      if (val !== undefined && val !== null && val !== "") {
        return { label: f, displayValue: typeof val === "object" ? JSON.stringify(val) : String(val) };
      }
    }
  ...
  const dv  = (val !== undefined && val !== null)
...
function buildOverlay(parsed, regexResult, specs, rawJson) {
  ...
  grid.style.cssText =
    `display:flex;align-items:stretch;border-bottom:1px solid ${th.sep};`;
  ...
  specs.forEach((spec) => {
    ...
    const lvlColor = (fieldName === "level") ? levelColor(resolved.displayValue) : null;
  ...
  // ── Collapsible json_payload viewer ──────────────────────────────────────
  if (rawJson) {
    const sep = document.createElement("div");
    sep.style.cssText = "padding:0 12px;";

    const toggle = document.createElement("div");
    toggle.style.cssText =
      `cursor:pointer;display:inline-flex;align-items:center;gap:6px;` +
      `color:${T().toggle};user-select:none;padding:4px 0 2px;font-size:12px;`;
    ...
    pre.style.cssText =
      `display:none;margin:6px 0 2px;padding:12px 16px;` +
      `background:${th.jsonBg};border:1px solid ${th.jsonBord};border-radius:6px;` +
      `color:${th.jsonFg};font-size:13px;line-height:1.7;overflow:auto;` +
      "max-height:440px;white-space:pre;word-break:normal;" +
    ...
    toggle.addEventListener("click", (e) => {
      ...
      sep.style.paddingBottom = open ? "0" : "6px";
      arrow.style.transform = open ? "rotate(0deg)" : "rotate(90deg)";
      toggle.style.color = open ? T().toggle : T().key;
    });
...
function insertOverlayAfterRow(row, overlay, fieldKey) {
  ...
  td.colSpan = 99;
  td.style.cssText = "padding:0;margin:0;border:none !important;overflow:visible;line-height:normal;";
...
function processTableRows(fields) {
  ...
  candidates.forEach((cell) => {
    ...
    if (parentRow && !seenRows.has(parentRow)) {
      ...
      if (cell.dataset.lfKey === fieldKey) { found++; return; }
      cell.dataset.lfKey = fieldKey;
      cell.querySelectorAll(`.${EXTRACT_CLASS}`).forEach((el) => el.remove());
  ...
  const dataRows = [...document.querySelectorAll("tbody tr")]
    .filter((tr) => !tr.classList.contains(EXTRACT_ROW_CLASS));

  dataRows.forEach((row, idx) => {
    const payload = cachedPayloads[idx];
    if (!payload) return;

    const parsed   = typeof payload === "object" ? payload : tryParseJson(String(payload));
...
function hideColumns({ fields = [] }) {
  ...
  fields.forEach((field) => {
    const idx = headers.findIndex((h) => h.textContent.trim().includes(field));
    if (idx < 0) return;
    const n = idx + 1;
    rules.push(
...
function stopExtract() {
  activeFields = [];
  if (extractObserver) { extractObserver.disconnect(); extractObserver = null; }
  document.querySelectorAll(`.${EXTRACT_ROW_CLASS}`).forEach((el) => el.remove());
...
chrome.runtime.onMessage.addListener((msg, _sender, sendResponse) => {
  try {
    switch (msg.action) {
      case "expandAll":      sendResponse(expandAll());              break;
      case "addFilter":      sendResponse(addFilter(msg));           break;
      case "addMultiFilter": sendResponse(addMultiFilter(msg));      break;
      case "removeFilter":   sendResponse(removeFilter(msg));        break;
      case "clearFilters":   sendResponse(clearFilters());           break;
      case "extractFields":  sendResponse(startExtract(msg.fields)); break;
      case "stopExtract":    sendResponse(stopExtract());            break;
      case "hideColumns":   sendResponse(hideColumns(msg));         break;
      case "showColumn":    sendResponse(showColumn());             break;
      case "setTheme": {
        activeTheme = msg.theme || "light";
        try { localStorage.setItem("lf_theme", activeTheme); } catch {}
        if (activeFields.length) {
          stopExtract();
