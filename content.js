...
// The preset's values for a field, the ones the loaded forms file lists for
// this host, plus whatever that field actually holds in the loaded logs — so a
// new project gets useful values before anyone lists them. cachedPayloads holds
// json_payload objects (with `kubernetes` grafted on), hence the stripped
// prefix.
function suggestValues(field) {
    const set = new Set([
        ...(lfActivePreset(suggestStore).values[field] || []),
        ...formsSuggestValues(field),
    ]);
...
function openPanel() {
    ...
    readStored(["extractFields", "extractAuto"]).then((d) => {
        ...
        renderChips();
        renderList();
        // Paint whatever the bootstrap already loaded, then re-read: a file
        // loaded from another tab lands in the store, not in this one's memory
        renderForms();
        renderLinks();
        loadFilterForms().then(() => {
            renderForms();
...
try {
    chrome.storage?.onChanged?.addListener((changes, area) => {
        ...
        // The forms store feeds the editor's suggestions too, so it is taken
        // whether or not the panel is up — only the repaint needs the panel.
...
function normalizeStoredForms(raw) {
    ...
    return {
        ...
        forms,
        links,
        // Already flattened when it was stored, so it does not go back through
        // the file normaliser with the forms above
        suggests: lfNormalizeSuggests(raw?.suggests),
        formsColor,
        linksColor,
    };
}
...
// Values the loaded forms file offers for a field on this host. Several groups
// can match one host (a shared `*` block beside a prod one), so they add up
// rather than the first one winning.
function formsSuggestValues(field) {
    const host = location.hostname;
    if (!lfHostMatches(filterForms.dns || [], host)) return [];
    const out = [];
    for (const s of filterForms.suggests || []) {
        if (!lfHostMatches(s.dns || [], host)) continue;
...
async function seedFormsFromBundle() {
    try {
        ...
        if (!url) return null;
        const { forms, links, suggests, dns, formsColor, linksColor } =
            lfNormalizeFilterForms(await (await fetch(url)).json());
...
// The one way in: read the store, seed the sample if it is still empty. Both
// the page bootstrap and the panel go through it, so neither can end up with a
// `filterForms` the other never saw.
async function loadFilterForms() {
    const d = await readStored([FORMS_KEY]);
    filterForms = normalizeStoredForms(d[FORMS_KEY]);
    // An older version seeded the sample before it carried `suggest`, and that
    // stale copy is what would answer the editor's completions — so the sample
    // is re-seeded when it is the sample that is stored. A file of your own is
    // never overwritten.
    const stale = filterForms.file === FORMS_BUNDLED && !filterForms.suggests.length;
    if (!filterForms.forms.length || stale) {
        const seeded = await seedFormsFromBundle();
        if (seeded) filterForms = seeded;
    }
    return filterForms;
}
...
function loadFormsFile(file) {
    ...
    reader.onload = () => {
        ...
        const { forms, links, suggests, skipped, dns, formsColor, linksColor } =
            lfNormalizeFilterForms(raw);
        // A file of nothing but links, or nothing but suggestion values, is a
        // legitimate file
        if (!forms.length && !links.length && !suggests.length)
        ...
        if (suggests.length) notes.push(`${suggests.length} suggest blocks`);
...
// A form may name the fields its rows should show — one button usually means
// one shape of log, and the extractor's standing list is rarely that shape. The
// list is not persisted: `extractFields` stays whatever the user chose, and the
// panel's own list comes back when the form is cleared.
let preFormFields = null;

function syncFormFields(form) {
    const fields = form?.fields || [];
    if (!fields.length) return restorePreFormFields();

    if (preFormFields === null)
        preFormFields = (activeFields.length ? activeFields : panelFields).slice();
    panelFields = fields.slice();
    renderChips();
    renderList();
    applyExtract({ fields: panelFields });
}

function restorePreFormFields() {
    if (preFormFields === null) return;
    const back = preFormFields;
    preFormFields = null;
    panelFields = back.slice();
    renderChips();
    renderList();
    if (back.length) applyExtract({ fields: back });
    else stopExtract();
}
...
function applyForm(form, isActive) {
    ...
    if (lucene.ok && editorTA) {
        ...
        const shown = form.fields?.length ? ` · ${form.fields.length} fields` : "";
        const note = `Ran · ${form.name}${dropped}${shown}`;
        return lfToast(note, dropped || shown ? 4000 : 2200);
    }
    ...
    if (form.fields?.length) notes.push(`${form.fields.length} fields`);
...
(async function initPageUI() {
    ...
    watchQueryBar();
    mountLauncher();
    // The forms file also carries the editor's value suggestions, and the
    // editor works with the panel shut — so the store is read (and seeded) here
    // rather than waiting for the panel to open.
    await loadFilterForms();
    let open = "0";
    try {
